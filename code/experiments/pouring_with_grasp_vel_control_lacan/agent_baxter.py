...
class AgentBaxter(Agent):
    ...
    def sample(self, policy, condition, verbose=True, save=True, noisy=True):
        ...
        if self.take_video:
            ...
            rospy.sleep(self._hyperparams['dt'])
            xx, image = self._get_current_state(t)
            self.rgb_writer.append_data(image)
            rospy.sleep(self._hyperparams['dt'])
            xx, image = self._get_current_state(t)
        ...
        self.axes[0, 1].clear()
        self.axes[0, 1].set_title('pixel_feature')
        self.axes[0, 1].plot(np.array(self.feature_traj)[:],c='r', label='x')
    ...
    def _get_current_state(self, t):
        ...
        # DON Features
        if self._hyperparams['anchor_id'] not in all_masks:
            ...
            image_feature = a_pred / 100000.0
            # image_feature = a_pred / len(a_pred) / 100000.0
            print(image_feature)
        ...
        cv2.imshow('blob', img_)


        self.feature_traj.append(image_feature)
        self.feature_traj[-1] = np.mean(np.array(self.feature_traj[-4:]), axis=0)
