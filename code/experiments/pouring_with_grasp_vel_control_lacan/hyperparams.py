...
for tt in range(agent['T']):
    ...
    if not agent['demo_centroids_precomputed']:
        ...
        all_centroids, all_masks, _ = get_mrcnn_features(mrcnn, img, dimg * DEPTH_SCALE, target_ids, class_names, ax=ax, viz_image=img)
        curr_masks = {}
...
cost_wt = np.ones(SENSOR_DIMS[IMAGE_FEATURE]) * 500
visual_cost = {
    'type': CostVisual,
    'l1': 1.0,
    'l2': 0.001,
