...
class AgentBaxter(Agent):
    ...
    #     # print(cur_gripper_pos, U[-1])
    #     self._trial_gripper.command_position(cur_gripper_pos + U[-1]*5)
    ...
    def _step_taskspace(self, U, curr_time):
        ...
        if self.gripper_movement_cnt % int(self._hyperparams['T'] / self._hyperparams['T']) == 0:
    ...
    def _init_sample(self, condition, feature_fn=None):
        ...
        gripper_binary = np.array([int(np.linalg.norm(p3d_l - p3d_r) > self._hyperparams['gripper_threshold'])])

        sample.set(JOINT_ANGLES, np.asarray(q), t=0)
        sample.set(JOINT_VELOCITIES, np.asarray(dq), t=0)
        sample.set(END_EFFECTOR_POINTS, np.concatenate([p3d_l, p3d_r, gripper_binary / 10.0]), t=0)
    ...
    def _get_current_state(self):
        ...
        X = np.concatenate([q, dq, pos, p3d_l, p3d_r, gripper_binary / 10.0])
    ...
    def _set_sample(self, sample, X, t, condition, feature_fn=None):
        ...
        sample.set(END_EFFECTOR_POINTS, np.concatenate([p3d_l, p3d_r, gripper_binary / 10.0]), t=t+1)
