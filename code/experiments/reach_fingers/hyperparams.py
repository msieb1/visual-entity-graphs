...
SENSOR_DIMS = {
    JOINT_ANGLES: 7,
    JOINT_VELOCITIES: 7,
    END_EFFECTOR_POINTS: 3 * EE_POINTS.shape[0] * 2 + 1,
    OBJECT_POSE: 7,
...
for i in range(agent['T']):
    ...
    mask = np.zeros(depth[i*step_size].shape, dtype=np.uint8)
...
cx_l = (1.610e-01 + 0.00301e-01) / 2.0 - 0.05
cy_l = (-1.063e-01 + 2.169e-02) / 2.0 + 0.08
cx_r = cx_l + 0.03
cy_r = cy_l

cost_tgt = np.hstack([-R*np.sin(2*math.pi*tt / agent['T']) + cx_l,-R*np.cos(2*math.pi*tt / agent['T']) + cy_l, 0*tt + 0.32, -R*np.sin(2*math.pi*tt / agent['T']) + cx_r, -R*np.cos(2*math.pi*tt / agent['T']) + cy_r, 0*tt + 0.32, 0*tt])
print(cost_tgt)
