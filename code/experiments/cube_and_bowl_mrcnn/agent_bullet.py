...
from gps.agent.bullet.bullet_utils import get_view_embedding, list_add, tinyDepth, BlobDetector
...
# Mask RCNN imports
MASK_RCNN_ROOT_PATH = None # SET PATH appropriately#
sys.path.append(MASK_RCNN_ROOT_PATH)
...
class AgentBullet(Agent):
    ...
    def get_state(self, t):
        ...
        object_p3d = np.array(self.getObjectPose(self.h.cubeid))

        rgb_crop, depth_crop = self.get_images(0)
        # rgb_crop, depth_crop = self.get_images_corrupted([[0, -1, 80, 82]])
        ...
        if fig is not None:
            ...
            mrcnn_image = np.array(fig.canvas.renderer._renderer)
        else:
            mrcnn_image = None
        ...
        if self.ptconf.COMPUTE_TCN:
            ...
            emb_normalized, a_pred = get_view_embedding(self.tcn, np.repeat(resized_image, 10, 0), resized_image,use_cuda=self.ptconf.USE_CUDA)
            # print(np.linalg.norm(demo_emb_normalized - emb_normalized))
            # plt.imshow(demo_frames[t+1])
            # plt.show()
            IMAGE_FEATURE = emb_normalized
        else:
        ...
        for key in curr_centroids.keys():
            if mrcnn_centroids[key] is None:
                ...
                self.mrcnn_centroids_last_known[key] = curr_centroids[key]

        centroid_diff = np.zeros(7,)
        cube_centroid_normalized = pixel_normalize(copy(curr_centroids[self.target_ids[0]]), max_x=self.camconf.IMG_W, max_y=self.camconf.IMG_H, max_z=255)
        bowl_centroid_normalized = pixel_normalize(copy(curr_centroids[self.target_ids[1]]), max_x=self.camconf.IMG_W, max_y=self.camconf.IMG_H, max_z=255)
        ...
        print("cost current step {}: {}".format(t+1, np.linalg.norm(centroid_diff - tgt )))
        # set_trace()
        # anchor_object_p3d[:3] /= self.cost_tgt_std[:3]
        # Collect all states
        # stateX = np.concatenate([np.array(joint_pos), np.array(joint_vel), np.abs(np.array(eepos) - object_p3d[:3]),IMAGE_FEATURE, centroid_diff]).flatten()
        stateX = np.concatenate([np.array(joint_pos), np.array(joint_vel), np.abs(np.array(eepos) - object_p3d[:3]),IMAGE_FEATURE, centroid_diff, anchor_object_p3d]).flatten()
    ...
    def get_images_corrupted(self, occlusion_inds, view=0):
        """
        Puts zebra stripes into image:
        occlusion_inds = [row_start, row_end, col_start, col_end]
        """
        cur = time.time()
        object_p3d = self.getObjectPose(self.h.cubeid)[0:3]
        #self.change_camera_target([object_p3d] * len(self.viewMatrices))
        ...
        for occs in occlusion_inds:
            rgb_img[occs[0]:occs[1], occs[2]:occs[3], :] = 0.0

        # kpts, im_w_kpts = self.blob_detector.get_blobs(rgb_img)
        return rgb_img, depth_img
