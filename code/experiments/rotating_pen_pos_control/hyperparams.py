...
from gps.utility.utils import pixel_normalize, compute_view_pose_embedding, resize_frame, load_pixel_model, compute_pixel_centroid_and_rotation_difference, compute_pixel_target_distances, compute_pixel_query_points, grabcut, depth_to_pc, depth_to_mask
...
IMAGE_CHANNELS = 3
TRUNCATE_FRONT = 30   # 30, 70 for picking and placing (square on ring)
TRUNCATE_BACK = 30
################
...
MRCNN_WEIGHTS_FILE = join(MRCNN_MODEL_DIR, 'pens/mask_rcnn_baxter_0003.h5')

### Demo Parameters ###
DEMO_NAME = 'pen_red_thick_4_view0'
TASK_NAME = 'rotate'
...
agent = {
    ...
    'demo_finger_traj': [],
    'u0': None,
    'set_action_to_zero': np.array([1,1, 1, 1, 1]), # x,y,z,rot,gripper
    'max_velocity': 0.02,
...
for tt in range(agent['T']):
    ...
    feature_pts = compute_pixel_query_points(feature_model, mask, agent['n_max_correspondences'], img)
    points_tgt.append(feature_pts)
...
cost_wt[[2, 5]] *= 0.0 # depth was halfed during deprojection
cost_wt *= 1
cost_tgt[:, 2] = -0.01
cost_tgt[:, 5] = -0.01
# cost_wt[-1] = 0
...
# algorithm['cost'] = {
#     'type': CostSum,
#     'costs': [state_cost_geom, fk_state_cost_geom, visual_cost, torque_cost],
#     'weights': [1.0, 0.0, 1.0, 0.1],
# }
...
algorithm['cost'] = {
    'type': CostSum,
    'costs': [fk_state_cost_geom,visual_cost, torque_cost],
    'weights': [1.0, 10.0, 0.1],
}
