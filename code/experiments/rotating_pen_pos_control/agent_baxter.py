...
class AgentBaxter(Agent):
    ...
    def sample(self, policy, condition, verbose=True, save=True, noisy=True):
        ...
        for t in range(self.T):
            ...
            b_U[3] = np.clip(b_U[3], -0.3, 0.3)  # clip rotation
    ...
    def _get_current_state(self, t):
        ...
        # MRCNN values
        all_centroids, all_masks, all_boxes = get_mrcnn_features(self.mrcnn, image, dimage_raw * DEPTH_SCALE, self.target_ids, self.class_names, ax=None)
        all_centroids[1] = np.array([[79, 219, -0.01]])
        ...
        try:
            ...
            # box = all_boxes[self._hyperparams['anchor_id']]
            # valid_target_ind[box[0]:box[2], box[1]:box[3]] = 1

            a_pred = np.array(self.feature_fn(self.feature_model, 
                np.reshape(self._hyperparams['cost_tgt_image_feature'][t], [-1, 2]), self._hyperparams['cost_tgt_image'][t], valid_target_ind,  image, self._hyperparams['n_max_correspondences'], 
                query_centroid=self._hyperparams['all_object_traj'][self._hyperparams['anchor_id']][t][:-1] , target_centroid=self.all_object_traj[self._hyperparams['anchor_id']][-1][:-1],debug=True) ) 
            # a_pred = np.array(self.feature_fn(self.feature_model, 
            ...
            # image_feature = np.mean(a_pred, axis=0)
            image_feature = np.sum(a_pred) / len(a_pred)
            print(image_feature)
            # print(image_feature)
            image_feature /= 1000.0
        ...
        self.feature_traj[-1] = np.mean(np.array(self.feature_traj[-6:]), axis=0)
