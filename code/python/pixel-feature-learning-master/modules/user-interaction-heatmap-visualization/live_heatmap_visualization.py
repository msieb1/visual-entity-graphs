...
class HeatmapVisualization(object):
    ...
    def _get_new_images(self):
        ...
        # self.img1_pil = Image.open('/home/zhouxian/ppp/7110.jpg')
        # self.img2_pil = Image.open('/home/zhouxian/ppp/334.jpg')
    ...
    def _compute_descriptors(self):
        ...
        for network_name, dcn in self._dcn_dict.iteritems():
            ...
            # self._res_a[network_name] = dcn.forward_single_image_tensor_firstlayers(self.rgb_1_tensor).data.cpu().numpy()
            # self._res_b[network_name] = dcn.forward_single_image_tensor_firstlayers(self.rgb_2_tensor).data.cpu().numpy()
    ...
    def find_best_match(self, event,u,v,flags,param):
        ...
        for network_name in self._dcn_dict:
            ...
            blended = cv2.addWeighted(self.img2_gray, alpha, heatmap, beta, 0) * 255
            blended = 255 - blended.astype(np.uint8)
            blended = cv2.applyColorMap(blended, cv2.COLORMAP_JET)
