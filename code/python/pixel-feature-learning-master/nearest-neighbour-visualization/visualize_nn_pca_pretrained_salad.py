...
def saveOrLoadPCA(src_dir, dim=128):
    ...
    picklepath = os.path.join(src_dir, 'pca_pretrain_%d.pkl'%(dim))
    ...
    source_list = np.random.choice(source_reshaped.shape[0], PCA_sample_num)

    # pca_data = np.concatenate([target_reshaped[target_list, ...], source_reshaped[source_list, ...]], axis=0)
    pca_data = source_reshaped[source_list]
...
def loadDataBuildTree():
    ...
    source_reshaped_path = os.path.join(src_dir, 'features_proj32.npy')#'features_syn_proj128.npy')
    target_reshaped_path = os.path.join(tar_dir, 'features_proj32.npy')#'features_city_proj128.npy')
    with open(os.path.join(src_dir, 'image_files_pretrain.pkl'), 'rb') as f:
        source_names = pickle.load(f)
    with open(os.path.join(tar_dir, 'image_files_pretrain.pkl'), 'rb') as f:
        target_names = pickle.load(f)

    if os.path.exists(source_reshaped_path):
        print('loading source projected 32 features')
        source_reshaped = np.load(source_reshaped_path)
        source_num, source_h, source_w, source_dim = (800, 60, 80, 32)
    else:
        source_reshaped = None
    # source_reshaped = None

    if os.path.exists(target_reshaped_path):
        print('loading tar projected 32 features')
        target_reshaped = np.load(target_reshaped_path)
        target_num, target_h, target_w, target_dim = (2, 60, 80, 32)
        #if 'val' in tar_dir:
    ...
    if target_reshaped is None:
        ...
        target_feature_raw = np.load(os.path.join(tar_dir, 'features_pretrain.npy'))
        print('using all target imgs. as testing.')
        target_feature = target_feature_raw#[:10, ...]
        if source_feature is None:
            print('reading source features...')
            source_feature = np.load(os.path.join(src_dir, 'features_pretrain.npy'))
