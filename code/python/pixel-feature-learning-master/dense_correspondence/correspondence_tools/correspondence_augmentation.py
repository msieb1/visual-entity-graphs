...
def random_domain_randomize_background(image_rgb, image_mask, from_provided_imgs=False, img_src=None):
    """
    ...
    if random.random() < 0.5:
        ...
        return domain_randomize_background(image_rgb, image_mask, from_provided_imgs, img_src)


def domain_randomize_background(image_rgb, image_mask, from_provided_imgs=False, img_src=None):
    """
    ...
    random_rgb_image = get_random_image(image_rgb_numpy.shape, from_provided_imgs, img_src)
...
def get_random_image(shape, from_provided_imgs=False, img_src=None):
    """
    ...
    :rtype: np.ndarray
    """
    if from_provided_imgs and random.random()<0.5:
        img_files = glob.glob(img_src)
        return np.array(Image.open(np.random.choice(img_files)).resize([shape[1], shape[0]]))
    else:
        if random.random() < 0.5:
