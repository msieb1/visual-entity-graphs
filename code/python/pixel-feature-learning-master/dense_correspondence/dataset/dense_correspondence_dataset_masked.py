...
class DenseCorrespondenceDataset(data.Dataset):
    ...
    def set_parameters_from_training_config(self, training_config):
        ...
        if (self.mode == "train") and ('domain_randomize_use_img' in training_config['training'].keys()) and (training_config['training']['domain_randomize_use_img']):
            logging.info("enabling domain randomization using provided images")
            self._domain_randomize_use_img = True
            self._background_rand_img_src = training_config['training']['background_rand_img_src']
        else:
            self._domain_randomize_use_img = False
            self._background_rand_img_src = None
