...
class DenseCorrespondenceNetwork(nn.Module):
    ...
    def forward_single_image_tensor_firstlayers(self, img_tensor):
        """
        Simple forward pass on the network.
        ...
        firstlayers = torch.nn.Sequential(*list(self.fcn.resnet34_8s.children())[:5])
        firstlayers.eval()
        res = nn.functional.upsample_bilinear(input=firstlayers.forward(img_tensor), size=(480, 640)) # shape [1,D,H,W]
        # print "res.shape 1", res.shape
    ...
    def from_config(config, load_stored_params=True):
        ...
        fcn = resnet_dilated.Resnet34_8s_pretrained(num_classes=config['descriptor_dimension'])
