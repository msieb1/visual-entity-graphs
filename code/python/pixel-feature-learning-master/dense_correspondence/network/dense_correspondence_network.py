...
class DenseCorrespondenceNetwork(nn.Module):
    ...
    def forward_multilabel(self, img_tensor):
        return self.fcn.forward_multilabel(img_tensor)
    ...
    @staticmethod
    def from_config(config, load_stored_params=True, cotrain=False, cotrain_num_classes=None):
        """
        ...
        if cotrain:
            fcn = resnet_dilated.Resnet34_8s_cotrain(num_classes=config['descriptor_dimension'], cotrain_num_classes=cotrain_num_classes)
        else:
