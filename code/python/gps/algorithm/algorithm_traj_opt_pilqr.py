...
class AlgorithmTrajOptPILQR(Algorithm):
    ...
    def _stepadjust(self, m):
        ...
        LOGGER.info('Trajectory step: ent: %f cost: %f -> %f',
                     ent, pmc, cmc)
        ...
        LOGGER.info('Previous cost: Laplace: %f MC: %f', np.sum(pdpc), pmc)
        LOGGER.info('Predicted new cost: Laplace: %f MC: %f', np.sum(pdcc), cmc)
        LOGGER.info('Actual new cost: Laplace: %f MC: %f', np.sum(cdcc), cmc)
