...
COLORS = visualize.random_colors(8)
...
### MRCNN Processing ###
def get_mrcnn_features(mrcnn, image, depth_rescaled, target_ids, class_names, ax=None, viz_image=None):
    # image_ = np.copy(image)
    # image_[:30, :, :] *= 0 # TODO: This is a hack !!!! get rid of it at some point 
    # image = image_
    results = mrcnn.detect([image], verbose=0)
    r = results[0]
    encountered_ids = []
    all_cropped_boxes = []
    all_boxes = dict()
    ...
    for i, box in enumerate(r['rois']):
        ...
        all_boxes_unordered.append(box)
        masked_depth = depth_rescaled * r['masks'][:, :, i]
        masked_depth = masked_depth[np.where(masked_depth > 0)]
        z = np.median(np.sort(masked_depth.flatten())) # TODO: Investigate hack here to avoid 0 depth values
        x, y = utils.get_box_center(box) # image space , so x is width, and y is height

        all_centroids_unordered.append([x, y, z])
        all_masks_unordered.append(r['masks'][:, :, i])
        filtered_inds.append(i)
    filtered_inds = np.array(filtered_inds)
    all_cropped_boxes = np.array(all_cropped_boxes)
    all_centroids_unordered = np.array(all_centroids_unordered)

    for i in range(all_cropped_boxes.shape[0]):
        all_centroids[encountered_ids[i]] = all_centroids_unordered[i][None, ...] # 1 by 3 array of x,y,z
        all_masks[encountered_ids[i]] = all_masks_unordered[i]
        all_boxes[encountered_ids[i]] = all_boxes_unordered[i]
       
    # all_centroids = np.asarray([val for key, val in all_centroids.items()])
    # all_masks = np.asarray([val for key, val in all_masks.items()])

    if ax is not None:
        ax.clear()
        try:
            if viz_image is not None:
                ax = visualize.display_instances(viz_image, r['rois'][filtered_inds], r['masks'][:, :, filtered_inds], r['class_ids'][filtered_inds],            
                            class_names, r['scores'][filtered_inds], ax=ax, colors=COLORS)
            else:
            ...
            if viz_image is not None:
                ax = visualize.display_instances(viz_image, r['rois'], r['masks'], r['class_ids'],            
                            class_names, r['scores'], ax=ax, colors=COLORS)
            else:
    ...
    return all_centroids, all_masks, all_boxes
