...
class AgentBullet(Agent):
    ...
    def __init__(self, hyperparams):
        ...
        with tf.device('/device:GPU:1'):
            ...
            self.mrcnn.load_weights(gconf.WEIGHTS_FILE_PATH, by_name=True)
    ...
    def get_state(self, t):
        ...
        bowl_centroid_normalized = pixel_normalize(curr_centroids[self.target_ids[1]], max_x=self.camconf.IMG_W, max_y=self.camconf.IMG_H, max_z=255)
        object_p3d[:3] = cube_centroid_normalized - bowl_centroid_normalized # Cube - Bowl  (# Look at Mask RCNN General config for ID order)
        # object_p3d -= self.cost_tgt_mean
        # object_p3d[:3] /= self.cost_tgt_std[:3]

        anchor_object_p3d = copy(object_p3d)
        anchor_object_p3d[:3] = curr_centroids[self.target_ids[1]]


        cube_p3d_gt = np.asarray(self.objects_centroids_mrcnn['5'][61/20*t])
        bowl_p3d_gt = np.asarray(self.objects_centroids_mrcnn['4'][61/20*t])
        ...
        # print("tgt - crnt: {}".format(np.abs(object_p3d[:3] - object_p3d_gt)))
        ...
        # Collect all states
        # stateX = np.concatenate([np.array(joint_pos), np.array(joint_vel), np.abs(np.array(eepos) - object_p3d[:3]),tcn_embedding, object_p3d]).flatten()
        stateX = np.concatenate([np.array(joint_pos), np.array(joint_vel), np.abs(np.array(eepos) - object_p3d[:3]),tcn_embedding, object_p3d, anchor_object_p3d]).flatten()
    ...
    def step2(self, action):
        for i in range(50):
          self.o.kukaobject.applyAction(action)
          p.stepSimulation()
          time.sleep(0.001)
    ...
    def get_images(self, view=0):
        ...
        test_depth = plt.imread('/home/msieb/projects/gps-lfd/demo_data/bowl/depth/16_0.png')
