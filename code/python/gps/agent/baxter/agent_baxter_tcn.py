...
from gps.proto.gps_pb2 import JOINT_ANGLES, JOINT_VELOCITIES, \
        ...
        END_EFFECTOR_POINT_JACOBIANS, ACTION, NOISE, IMAGE_FEATURE, RGB_IMAGE
...
from baxter_utils import img_subscriber, resize_frame, load_model, IMAGE_SIZE
...
class AgentBaxterTCN(Agent):
    ...
    def __init__(self, hyperparams, init_node=False):
        ...
        self.n_emb = self._hyperparams['sensor_dims'][IMAGE_FEATURE]
        ...
        self.tcn = load_model(model_path=model_path)

        # Take some ramp images to allow cams to adjust for brightness etc.
        for i in range(100):
            # Get frameset of color and depth
            for img_subs in self.img_subs_list:
                color_image = img_subs.img
                resized_image = resize_frame(color_image, IMAGE_SIZE)[None, :]
            print('Taking ramp image %d.' % i)
        image_buffer_curr = np.zeros((len(self.views_ids), 3, IMAGE_SIZE[0], IMAGE_SIZE[1]))
        for i in range(len(self.views_ids)):
            image_buffer_curr[i] = resized_image
    ...
    def sample(self, policy, condition, verbose=True, save=True, noisy=True):
        ...
        for t in range(self.T):
            ...
            b_U[:-1] = np.clip(b_U[:-1], vel0, vel1)        
            b_U[-1] = np.clip(b_U[-1], -0.3, 0.3)
    ...
    def _init_sample(self, condition, feature_fn=None):
        ...
        image_buffer_curr = np.zeros((len(self.views_ids), 3, IMAGE_SIZE[0], IMAGE_SIZE[1]))
        for i, img_subs in enumerate(self.img_subs_list):
            image = img_subs.img        
        ...
        resized_image = resize_frame(image, IMAGE_SIZE)[None, :]
        output_normalized, output_unnormalized, soft = self.tcn(torch.Tensor(image_buffer_curr).cuda())
        # image_buffer.append(image[:,:,::-1])
        sample.set(IMAGE_FEATURE, output_normalized.detach().cpu().numpy().flatten(), t=0)
    ...
    def _set_sample(self, sample, X, t, condition, feature_fn=None):
        ...
        # # image_buffer.append(image[:,:,::-1])
        # sample.set(IMAGE_FEATURE, output_unnormalized.detach().cpu().numpy()[0], t=t+1)
