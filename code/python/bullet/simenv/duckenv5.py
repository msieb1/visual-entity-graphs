...
PYBULLET_DATA_PATH = join(HOME_PATH, "libs/pybullet_data")
...
class UserEnv(Env):
    ...
    def load(self):
        ...
        h.duckid = p.loadURDF(join(PYBULLET_DATA_PATH,"duck_vhacd.urdf"), 0.9650000,-0.100000, 0.675000,0.000000,0.000000,0.707107,0.707107)
        h.cube2id = p.loadURDF(join(PYBULLET_DATA_PATH, 'cube_large.urdf'), 0.850000,-0.0000, 0.675000,0.000000,0.000000,0.707107,0.707107)
    ...
    def reset(self, reset_condition=None):
        ...
        # Reset object
        p.resetBasePositionAndOrientation(self.h.duckid, [0.9650000,-0.100000, 0.675000],
                                          p.getQuaternionFromEuler([math.pi/2, 0, math.pi/2]))

        time.sleep(0.5)
        orn = [0.70603903128, 0.708148792076, 0, 0]
        pos = [0.96, -0.100000, 0.802]
