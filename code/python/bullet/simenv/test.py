...
class UserEnv(Env):
    ...
    def load(self):
        ...
        h.bowlid = p.loadURDF(userdatapath('..', 'tools', 'model_normalized.urdf'), 1.050000,-0.500000,0.700000,0.707107,0,0,0.707107)
        # h.bowlid = p.loadSDF(userdatapath('..', 'tools', 'bag01', 'models', 'newsdf.sdf'))
        # h.cubeid = p.loadURDF(sysdatapath("cube_small.urdf"), 0.950000,-0.100000,0.700000,0.000000,0.000000,0.707107,0.707107)
        # h.ballid = p.loadURDF(sysdatapath("sphere_small.urdf"), 0.850000,-0.400000,0.700000,0.000000,0.000000,0.707107,0.707107)
    ...
    def objects(self):
        return (self.h.kuka, self.h.cubeid, self.h.ballid)
