...
class BulletEnv(Env):
    ...
    def initialize_world(self, all_traj, object_ids, object_reset_poses):
        ...
        self.init_policy = np.asarray(all_traj['2'])[:, [2,3,4, -1]]
        self.object_ids = [4, 5]
...
###### RL LAB API ##############3
    ...
    def action_space(self):
        return Box(low=-np.inf, high=np.inf, shape=(self.action_dimension,)) 

    @property
    def observation_space(self):
        """States: Block state, end_effector state, goal_state
        """
        return Box(low=-np.inf, high=np.inf, shape=(self.observation_dimension,))  
